...
export default function AppSidebar() {
  ...
  return (
    <aside className=" fixed top-0 left-0 z-20 w-24 h-screen bg-sky-200 flex flex-col justify-between items-center text-sky-900">
      {/* top logo */}
      <img to="/app" src={logo} alt="TSE" className="h-14 mt-4" onClick={() => navigate("/app")}/>

      {/* nav items */}
      <nav className="flex-1 w-full flex flex-col items-center">
        <Item to="/app/cuenta" icon={UserCircle} label="Cuenta" />
        <Item to="/app/logistica" icon={Car} label="Logística de transporte" />
        <Item
